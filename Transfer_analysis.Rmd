...
fitted <- data.frame(matrix(ncol=12,nrow=nrow(test)))
colnames(fitted) <- c("mod1","mod2","mod3","mod4","mod5","mod6","mod7","mod8","mod9","mod10","mod11","mod12")
fitted[,1] <- predict(mod1, test)
fitted[,2] <- predict(mod2, test)
fitted[,3] <- predict(mod3, test)
fitted[,4] <- predict(mod4, test)
fitted[,5] <- predict(mod5, test)
fitted[,6] <- predict(mod6, test)
fitted[,7] <- predict(mod7, test)
fitted[,8] <- predict(mod8, test)
fitted[,9] <- predict(mod9, test)
fitted[,10] <- predict(mod10, test)
fitted[,11] <- predict(mod11, test)
fitted[,12] <- predict(mod12, test)
...
for(i in 1:12){
  mod_rmse[i,1] <- i
  mod_rmse[i,2] <-  round(caret::RMSE(test$bpm2,fitted[,i]),4)
}
...
The RMSE from mod1 is `r round(mod_rmse[1,2],4)`
...
The RMSE of extreme gradient boosting training is `r rmsex`. It appears that the RMSE of the basic model (mod1) is the best, followed by LOOCV, LOO-KNN, KNN, and finally XGBT.
