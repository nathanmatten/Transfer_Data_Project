...
tran_final <- tran_yay %>% 
  ...
  select(pid,player,year,tran_year,old_conference,old_team,new_conference,new_team,BPM,conf_rating) %>% 
  arrange(old_conference,old_team,new_conference,new_team,player,year)
...
BootKNN(tran_final,classes = c(tran_final$year,tran_final$BPM,tran_final$conf_rating),sub = tran_sub, nboot = 1000, method = "knn")
```
