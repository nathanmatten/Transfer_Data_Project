...
tran_stats <- rbind(tran_2015,tran_2016,tran_2017,tran_2018,tran_2019,tran_2020,tran_2021,tran_2022) %>% 
  select(player,GP,Min_per,ORtg,usg,eFG,TS_per,ORB_per,DRB_per,AST_per,TO_per,FTM,FTA,FT_per,twoPM,twoPA,twoP_per,TPM,TPA,TP_per,blk_per,stl_per,ftr,yr,porpag,pid,BPM)
``` 
...
t2 <- merge(tran_team, tran_stats, by.x = "player", by.y = "player") %>% 
  select(player,old_team,old_conference,new_team,new_conference,grade,year,pid) %>% 
  arrange(pid)
...
tran_yay <- merge(t2,tran_stats, by.x = "pid", by.y = "pid") %>% 
  ...
  select(-player.y,-player.x) %>% 
  arrange(old_conference,year,old_team,new_conference,new_team,player)
```
